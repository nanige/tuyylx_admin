      ...
      <Row>
        ...
        <div class="steps-one" v-if="steps == 0">
          <Form ref="lineReleaseForm" :model="lineReleaseForm" :rules="lineReleaseRuleVali"  :label-width="95">
            ...
            <Form-item label="过期时间">
              <Date-picker type="date" @on-change="getExpiredate" placeholder="选择日期" style="width: 200px"></Date-picker>
            </Form-item>
            <Form-item label="出发城市">
              <div class="orea-wrap orea-go">
                <div class="orea-wrap-cent" v-for="(item,key) in guoNeiList" :key="item.menuName">
                  <div class="orea-wrap-left fl">
                    <span>{{item.menuName}}</span>
                  </div>
                  <div class="orea-wrap-right fl">
                    <Checkbox-group v-model="goCityArr">
                      <Checkbox v-for="value in item.childList" :label="value.menuCode"  :key="value.menuName"><span>{{value.menuName}}</span></Checkbox>
                    </Checkbox-group>
                  </div>
                </div>
              </div>
            ...
            <Form-item label="目的地城市" class="mudiPosi">
              <Select  placeholder="请选择省份" @on-change="changeShengFen" v-model="shengCode" class="shengfen" v-if="tabName == '省内游'" >
                <Option :value="item.provinceCode" v-for="item in shengNeiList" :key="item.provinceName">{{item.provinceName}}</Option>
              </Select>
              <Tabs value="name1" v-model="tabName" :animated="true" >
                <Tab-pane :label="item.dtonName" :name="item.dtonName" v-for="item in maxAreaList" :key="item.dtonName" >
                  <div class="orea-wrap" v-if="tabName == '境外游'">
                    <div class="orea-wrap-cent" v-for="(value,key) in guoWaiList" :key="value.menuName" >
                      <div class="orea-wrap-left fl">
                        <span>{{value.menuName}}</span>
                      </div>
                      <div class="orea-wrap-right fl">
                        <Checkbox-group v-model="destCityArr">
                          <Checkbox :label="minvalue.menuCode" v-for="minvalue in value.childList" :key="minvalue.menuName"><span>{{minvalue.menuName}}</span></Checkbox>
                        </Checkbox-group>
                      </div>
                    </div>
                  </div>
                  <div class="orea-wrap" v-if="tabName == '国内游'">
                    <div class="orea-wrap-cent" v-for="(value,key) in areaList" :key="value.menuName" >
                      <div class="orea-wrap-left fl">
                        <span>{{value.menuName}}</span>
                      </div>
                      <div class="orea-wrap-right fl">
                  ...
                  <div class="orea-wrap" v-if="tabName == '省内游'">
                    <div class="orea-wrap-cent" v-for="(value,key) in shengQuList" :key="123" >
                      ...
                      <div class="orea-wrap-right fl">
                        <Checkbox-group v-model="destCityArr">
                          <Checkbox :label="box.cityCode" v-for="box in value" :key="box.cityName">{{box.cityName}}</Checkbox>
                        </Checkbox-group>
                      </div>
                    </div>
                  </div>
            ...
            <Form-item label="线路主题">
              <Checkbox-group v-model="themeCodeArr">
                <Checkbox :label="item.themeCode" v-for="item in lineThemeData" :key="item.themeName"><span>{{item.themeName}}</span></Checkbox>
              </Checkbox-group>
            </Form-item>
            ...
            <Form-item label="标签" >
              <Checkbox-group v-model="labelCodeArr">
                <Checkbox :label="item.labelcode" v-for="item in labelList" :key="item.labeldesc">{{item.labeldesc}}</Checkbox>
              </Checkbox-group>
            </Form-item>
            <Form-item>
        ...
        <div v-if="steps == 1">
          <Tabs value="tripTab1" v-model="tabTwoTabValue">
            <Tab-pane :label="'第'+(index+1)+'天'" v-for="(value,index) in tripFormArr" :key="123" >
              <Form ref="lineReleaseForm" :model="tripForm" :rules="tripRuleVali" :label-width="80">
                <Form-item label="行程标题" prop="title">
                  <Input v-model="value.title" placeholder="请输入行程标题"></Input>
                </Form-item>
                <Form-item label="用餐">
                  <Checkbox-group v-model="value.mealsList">
                ...
                <Form-item label="住宿" >
                  <Radio-group v-model="value.isstay">
                    <Radio label="0">
                      <span>是</span>
                    </Radio>
                    <Radio label="1">
                ...
                <Form-item label="活动安排">
                  <vue-editor v-model="value.memo"></vue-editor>
                </Form-item>
                <Form-item label="附加项目">
                  ...
                  <table class="line-table-utia">
                    ...
                    <tbody>
                    <tr v-for="(item,index) in value.line55List" :key="123">
                ...
                <Form-item label="购物项目">
                  ...
                  <table class="line-table-utia">
                    ...
                    <tbody>
                    <tr v-for="(item,index) in value.line56List" :key="123">
                      <td><Input v-model="item.projectcode" placeholder="请输入..." ></Input></td>
                ...
                <Form-item label="附加产品">
                  ...
                  <table class="line-table-utia">
                    ...
                    <tbody>
                    <tr v-for="(item,index) in value.line8List" :key="123">
                ...
                <Form-item>
                  ...
                  <Button type="primary" @click="steps++">保存</Button>
                </Form-item>
              </Form>
            </Tab-pane>
          </Tabs>
        ...
        <!-- 第3步 -->
        <div v-if="steps == 2">
          <Button type="success" class="add-margin" @click="editLinePriceModal2 = true">添加新路线价格</Button>
          <Button type="success" class="add-margin" @click="preferenModalShow">线路优惠</Button>
          <Table :columns="columns3" :data="priceList"></Table>
          <Modal v-model="editLinePriceModal" :width="900" title="编辑线路价格" @on-ok="pricePlanDetermine" :mask-closable="false" @on-cancel="pricePlanCancel">
            <Row>
          ...
          <Modal v-model="editLinePriceModal2" :width="900" title="添加线路价格" @on-ok="pricePlanDetermine2" :mask-closable="false" >
            <Row>
              <Col span="22" class="margin-center">
              <Form ref="formValidate" :model="editLinePriceForm" :rules="pricePlanRuleVali" :label-width="130" >
                <Form-item label="名称" prop="pricedesc">
          ...
          <Modal title="特殊日期编辑" :width="1400" v-model="editSpecialModal" @on-ok="editSpecialDetermine" @on-cancel="editSpecialCancel" :mask-closable="false">
            <Row>
              <Col span="16">
                ...
                <Tabs type="card" value="name1" v-model="specialTab">
                  ...
                  <Tab-pane label="按号" name="name2">
                    <Checkbox-group v-model="editSpecialPriceForm.numList">
                      <Checkbox :label="item.day" v-for="item in specialDateObj.daysObj" :key="item.day"></Checkbox>
                    </Checkbox-group>
                  </Tab-pane>
                </Tabs>
                <div>
          ...
          <div class="text-align margin-top">
            <Button type="success" @click="steps++">保存</Button>
          </div>
        </div>
        <!-- 第4步 -->
        <div v-if="steps == 3">
          <Button type="primary" @click="getCheckboxArr">保存</Button>
          <Button type="success" @click="publishLineSubmit">发布线路</Button>
          <Row>
            <Col span="12">
              <!--<Tree :data="baseData" show-checkbox></Tree>-->
              <Checkbox-group v-model="baseCheckboxArr">
                <div class="tree-div" v-for="(item,index) in baseData" :key="item.cmpcode">
                  <Checkbox :label="item.cmpcode" :disabled="item.mark" @click="cmpCodeArr(item)"><span>{{item.title}}</span></Checkbox>
                  <div v-for="(item2,index) in item.children">
                    <Checkbox :label="item2.cmpcode"  :disabled="item2.mark"><span>{{item2.title}}</span></Checkbox>
                    <div v-for="(item3,index) in item2.children">
                      <Checkbox :label="item3.cmpcode"  :disabled="item3.mark"><span>{{item3.title}}</span></Checkbox>
                      <div v-for="(item4,index) in item3.children">
                        <Checkbox :label="item4.cmpcode"  :disabled="item4.mark"><span>{{item4.title}}</span></Checkbox>
                        <div v-for="(item5,index) in item4.children">
                          <Checkbox :label="item5.cmpcode"  :disabled="item5.mark"><span>{{item5.title}}</span></Checkbox>
                        </div>
                      </div>
                    </div>
                  </div>
            ...
            <Col span="12">
              <ul class="qudao-right">
                  <li v-for="(item,index) in baseCheckboxArr">
                    <span @click="delCheckboxArr(index)" class="close-span">X</span>
                    {{item}}
                  </li>
              </ul>
            </Col>
  ...
  export default {
    ...
    data () {
      return {
        ...
        editLinePriceModal2: false, // 添加编辑模态框
    ...
    methods: {
      ...
      oneSubmit (name) {
        if (this.$route.query.menuCode == 2001001001) {
          this.lineReleaseForm.lineTypecode = 1
        } else if (this.$route.query.menuCode == 2001002001) {
          this.lineReleaseForm.lineTypecode = 2
        }
        ...
        if (this.tabName === '省内游') {
          ...
          this.lineReleaseForm.natureCode = 1
        } else if (this.tabName === '国内游') {
          this.lineReleaseForm.dtonCode = 2
          this.lineReleaseForm.natureCode = 2
        } else if (this.tabName === '境外游') {
          this.lineReleaseForm.dtonCode = 3
          this.lineReleaseForm.natureCode = 3
        }
        this.self.oneSubmitForm({model: JSON.stringify(this.lineReleaseForm)})
      },
      oneReset (name) {
      ...
      tripSubmit () {  // 第2步提交表单
        if (this.mealsArr == []) {
          ...
          this.tripForm.meals = this.mealsArr.join(',')
        }
        this.tripForm.linecode = this.tripLineCode
        this.tripForm.day = this.tabTwoTabValue + 1
        console.log(this.tripForm)
        this.self.tripSubmitTwo({model: JSON.stringify(this.tripForm)})
      },
      // 第3步----------------
      preferenModalShow () {
      ...
      pricePlanDetermine2 () {
        this.editLinePriceForm.linecode = this.lineCode
        this.self.editLinePrice2(this.editLinePriceForm)
      },
      addSpecialTable () {
      ...
      // 发布线路
      publishLineSubmit () {
        this.self.publishLine({linecode: this.lineCode})
      },
      // 第4步
      getCheckboxArr () {
        this.baseCheckboxArr = this.selectQudaoList
        console.log(this.selectQudaoList)
      },
      delCheckboxArr (index) {
        this.baseCheckboxArr.splice(index, 1)
      },
      cmpCodeArr (data) {
        this.baseCheckboxArr
      }
    },
    mounted () {
      ...
      this.uploadFourUrl = this.api + '/biz/line7/upload.do'
      // 第5步
      this.self.channelList()
    }
  }
</script>
<style lang="less">
