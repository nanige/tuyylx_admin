         ...
         <div class="header-conditions">
           <Form-item class="role-label" label="角色ID" prop="roleId">
            ...
            <div class="role-perm">
              ...
              <div class="role-tab">
                <Tabs type="card">
                  <Tab-pane v-for="item in permList" :key="item" :label="item.menuDesc">
                    <div v-for="(item,index) in item.childMenuList" :key="item" >
                      <div class="role-add-title">
                        ...
                        <label class="tab-title" v-if="item.childMenuList == ''" >{{item.menuDesc}}<input  v-model="item.checked" type="checkbox"></label>
                      </div>
  ...
  export default {
    ...
    methods: {
      clickAddRolePerm () { //点击提交按钮
        ...
        for(let value of this.permList.entries()){
        ...
        this.self.addRolePerm(this.selectData) //添加角色
      }
