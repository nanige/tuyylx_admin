<template>
  <div>
    <Row>
      ...
      <div class="left-menu-madel" v-if="treePanel">
        ...
        <div class="content" style="background:#fff">
          <ul>
            <li v-for="item in menuForLeft" class="lv1" >

                                <span class="plv1" @click.stop="item.isOpen === true ? item.isOpen = false : item.isOpen = true" v-on:mouseenter="item.isShow = true" v-on:mouseleave="item.isShow = false">
      ...
      <div class="screen" :style="'height:' + height">
        ...
        <Form ref="search" v-if="tableShow" :model="search" class="zidingyi" style="margin: 25px 25px 0 0" :label-width="80">
          <Row>
            ...
            <Col span="7">
            <Form-item label="目的地:">
              <Input size="small" v-model="search.passwd"></Input>
            </Form-item>
            </Col>
            <Col span="7" offset="1">
    ...
    <Modal v-model="clustering" width="920">
      ...
      <div>
        ...
        <div class="allocation" >
          <!--v-if="paiTuanItem.isguide === '1'"-->
          <h1 class="text-align teamTile daoyoupeizhi"> - 导游分配 - </h1>

          <Form ref="allocation" :model="allocation" :label-width="80">
            <Row>
              <Col span="2" style="line-height: 35px">
                地陪导游：
              </Col>
              <Col span="2" style="line-height: 35px">
                {{paiTuanItem.localnum}}人
              </Col>
              <Col span="8" style="line-height: 35px">
                语种：{{paiTuanItem.language}}
              </Col>
              <Col span="10" offset="1">
                <Form-item label="选择导游：">

                  <Row>
                    <Col span="22">
                      <Input :value="value1" class="daoyouEnter"></Input>
                    </Col>
                    <Col span="2">
                    <Button type="ghost" class="daoyouBtn" @click="showDaoyou ('2')">···</Button>
                    </Col>
                  </Row>
            ...
            <Row>
              ...
              <Form-item label="选择导游：">
                ...
                <Row>
                  <Col span="22">
                  <Input :value="value2" class="daoyouEnter"></Input>
                  </Col>
                  <Col span="2">
                  <Button type="ghost" class="daoyouBtn" @click="showDaoyou ('1')">···</Button>
                  </Col>
                </Row>
            ...
            <Row>
              ...
              <Col span="22" style="line-height: 35px">
                <Input type="textarea" v-model="value" :rows="5" style="margin-top: 10px"></Input>
              </Col>
      ...
      <div slot="footer" class="text-align">
        <Button type="success" size="large" @click="paiTuanSub">提交</Button>
      </div>
    </Modal>
    <Modal v-model="daoyouList" width="820">
      <p slot="header" style="color:#fff;">
        <span>选择导游</span>
      </p>
      <div>
        <Table stripe :columns="daoyouCol" :data="daoyouData"
          @on-select = "daoYouOnSel"
          @on-selection-change="daoYouOnSel"
          @on-select-all = "daoYouOnSel"
          @on-select-cancel = "daoYouOnSel"
        ></Table>
      </div>
      <div slot="footer" class="text-align">
        <Button type="success" size="large" @click="queDingDaoYou (allDaoYou,guidetype)">确定</Button>
      </div>
    </Modal>
  </div>
...
<script>
  ...
  export default {
                  ...
                  h('Button', {
                    ...
                    on: {
                      click: () => {
                        ...
                        list.paiTuanInfo (this,{ groupno: params.row.groupno})
                      }
                    }
                  }, '派团'),
                  h('Button', {
                    ...
                    on: {
                      click: () => {
                        ...
                        list.paiTuanInfo (this,{ groupno: params.row.groupno})
                        list.paiTuanEditInfo (this,{ groupno: params.row.groupno})
                      }
                    }
                  }, '编辑')
                ])
    ...
    methods: {
      ...
      showDaoyou (type) {
          this.daoyouList = true
          this.guidetype = type

          switch (this.statusT) {
            case 'add':

              let languageItem = this.paiTuanItem.language.replace('，',',')
              list.getDaoyouList ( this,{nameStr: languageItem} )
                  break
            case 'edit':

              this.localcode.length = 0
              this.allcode.length = 0

              let languageItem1 = this.paiTuanItem.language.replace('，',',')

              if (type === '1') {
                  if (this.paiTuanItem2.localguide.indexOf(',') === -1) {
                    let arr = this.paiTuanItem2.localguide.split(':')
                    this.localcode.push(arr[0])

                    list.getDaoyouList2 ( this,{nameStr: languageItem1} )
                  } else {
                    let arrA = this.paiTuanItem2.allguide.split(',')
                    arrA.map( (el,i) => {
                      let arrB = el.split(':')
                      this.localcode.push(arrB[0])
                    })
                  ...
                  if (this.paiTuanItem2.allguide.indexOf(',') === -1) {
                    let arr = this.paiTuanItem2.allguide.split(':')
                    this.allcode.push(arr[0])
      ...
      paiTuanSub () {
          ...
          switch (this.statusT) {
            case 'add':
                this.self.submitPaiTuan (this.gmg1ModelStr)
                  break
            case 'edit':
              this.self.submitPaiTuanEidt (this.gmg1ModelStr)
                  break
            default:
                break
          }
      ...
      queDingDaoYou (item,guidetype) {

        if (guidetype === '1') {
          this.gmg1ModelStr.localguide = ''
          this.value2 = ''
          let local = ''
          ...
          item.map( (el,i) => {
            ...
            if (local === '') {
              local = local + el.guidecode + ':' + el.guidename
            } else {
              local = local  + ',' + el.guidecode + ':' + el.guidename
            }

            this.value2 = local
            this.gmg1ModelStr.localguide = local
          })
        } else if (guidetype === '2') {
          this.gmg1ModelStr.allguide = ''
          this.value1 = ''
          let local = ''
          ...
          item.map( (el,i) => {
            ...
            this.gmg1ModelStr.allguide = local
          })
        } else {
            return
        }
        this.daoyouList = false
      }
